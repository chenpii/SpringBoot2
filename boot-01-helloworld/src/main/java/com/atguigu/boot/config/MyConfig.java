...
/**
 * 1、配置类里面使用@Bean注解在方法上给容器注册组件，默认也是单实例。
 * 2、配置类本身也是组件
 * 3、proxyBeanMethods：代理bean方法
 * Lite(proxyBeanMethods = true)【每个@Bean方法被调用多少次返回的组件都是单实例的】
 ...
 *
 * @author chenpi
 * @create 2022-10-18 19:42
 */
@Import({User.class, DBHelper.class})
@Configuration(proxyBeanMethods = true) //告诉Spring 这是一个配置类 == 配置文件
